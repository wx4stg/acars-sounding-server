...
from redacted import makeSoundingDataset
...
def plotParams(profileData, ax):
    ...
    if not np.isnan(profileData.dcape_profile[0]):
        ...
        print(profileData.dcape_profile[0])
        ax.text(0.5, 0.6, f"DCAPE:\n{int(profileData.dcape.to('J/kg').magnitude)} J/kg, {downMax:.1f} m/s, {int(profileData.dcape_profile[0].data.to(units.degF).magnitude)}F", ha="center", va="top", transform=ax.transAxes, clip_on=False, zorder=5)
    else:
...
def plotSkewT(profileData, skew, parcelType="sb"):
    ...
    skew.plot(profileData.LEVEL, profileData.dcape_profile, "rebeccapurple", linestyle="--", markersize=3, zorder=6)
